...
Deno.serve(async (req) => {
  ...
  // ── 1. Onboarding nudge ──────────────────────────────────────────────────
  // New users (signed up 2–7 days ago) with 0 active rentals who haven't received this email.
...
<p><a href="${SITE_URL}" style="background:#1B2A4A;color:#F5F0E8;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin-top:8px">Browse the wardrobe →</a></p>
<p style="margin-top:24px;font-size:13px;color:#999">You're receiving this because you signed up at davenport.rentals. <a href="${SITE_URL}/unsubscribe">Unsubscribe</a></p>`
    const ok = await sendEmail(user.email, subject, html)
    if (ok !== false) { await logEmail(user.id, user.email, 'onboarding_nudge', subject); sent++ }
  }

  // ── 2. 3-month buyout nudge ───────────────────────────────────────────────
  // Active rentals that are ~90 days old (88–92 day window) and not yet bought out.
  ...
  const { data: rentals3m } = await supabaseAdmin
    .from('rentals')
...
<p><a href="${SITE_URL}/account" style="background:#1B2A4A;color:#F5F0E8;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin-top:8px">Buy it now →</a></p>
<p style="font-size:13px;color:#666;margin-top:16px">Or keep renting — the price keeps dropping the longer you hold it.</p>
<p style="margin-top:24px;font-size:13px;color:#999"><a href="${SITE_URL}/unsubscribe">Unsubscribe</a></p>`
    const ok = await sendEmail(profile.email, subject, html)
    if (ok !== false) { await logEmail(rental.user_id, profile.email, `buyout_nudge_3m_${rental.id}`, subject); sent++ }
  }

  // ── 3. 6-month loyalty nudge ─────────────────────────────────────────────
  // Active rentals ~180 days old (178–182 day window), not bought out.
  // Loyalty discount: 5% off the current buyout price.
  ...
  const { data: rentals6m } = await supabaseAdmin
    .from('rentals')
    .select('id, user_id, created_at, buyout_price_snapshot, size, piece:pieces(name, brand)')
    .eq('billing_active', true)
    .eq('bought_out', false)
...
<p><a href="${SITE_URL}/account" style="background:#1B2A4A;color:#F5F0E8;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin-top:8px">Claim your loyalty price →</a></p>
...
<p style="margin-top:24px;font-size:13px;color:#999"><a href="${SITE_URL}/unsubscribe">Unsubscribe</a></p>`
    const ok = await sendEmail(profile.email, subject, html)
    if (ok !== false) { await logEmail(rental.user_id, profile.email, `buyout_nudge_6m_${rental.id}`, subject); sent++ }
  }
